...
//     },
//   ];
//   // Links for CV, Thesis, and Projects
//   const resourceLinks = [
//     { label: "My CV", url: "/Shakil_Ahmed_Resume.pdf" },
//     { label: "Thesis Paper", url: "/SVASP.pdf" },
//   ];
//   const headingText = "Why Choose Me For Your Next Project ?";
              ...
              className={`flex items-center justify-start
                ${
                  infoItem.label === "Client"
                    ? "sm:justify-start"
                    : infoItem.label === "Projects"
                    ? "sm:justify-center"
                    : "sm:justify-end"
                } py-4 sm:py-6 w-8/12 px-4 sm:w-auto mx-auto sm:mx-0
                `}
              key={infoItem.label} // Unique key
              custom={{ duration: 2 }}
              variants={setVariants}
            >
              <div className="flex m-0 w-40 sm:w-auto">
          ...
          >
            {data?.skills &&
              data?.skills.split(",").map((skill) => {
                const trimmedSkill = skill.trim(); // Trim whitespace for safety
                return (
                  <motion.div
                    key={trimmedSkill} // Use skill name as key
                    className="w-full flex justify-center items-center"
                    variants={skillItemVariant}
                  >
                    <button className="whitespace-nowrap text-ellipsis overflow-hidden py-3 w-[160px] px-6 border-[2px] border-green-main bg-[#fff] text-[#000] font-semibold rounded-lg text-xl tracking-widest hover:shadow-green-main transition-all outline-none">
